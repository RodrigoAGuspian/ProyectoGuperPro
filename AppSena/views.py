...
def view_lista_fichas(request):
	...
	if ficha:
		...
		listaA = []
		for i in ficha:
			programa = Ficha.objects.filter(numeroFicha = i.numeroFicha)
			aprendiz = Persona.objects.filter(id__in = [i.persona.id for i in Persona_ficha.objects.filter(ficha__numeroFicha = i.numeroFicha)]).count()
			listaP.append(programa)
			listaA.append(aprendiz)
		PA = zip(listaP,listaA)
	else:
	...
	try:
		...
		if rp:
			...
			for x in rp:
				...
				_id = x.persona.id
				doc = x.persona.documentoIdentidad
				nom = x.persona.nombres
				ape = x.persona.apellidos
				eml = x.persona.usuario.email
				
				actividad = Permiso_persona.objects.filter(instructor=_id)

				if actividad:
					act = True
				
				p = _id,doc,nom,ape,eml,act
...
def view_lista_vigilantes(request):
	try:
		...
		rp=Rol_persona.objects.filter(rol=vg)
		if rp:
			...
			for x in rp:
				...
				actividad = Permiso_persona.objects.filter(vigilante=_id)
	...
	try:
		...
		if rp:
			...
			for x in rp:
				...
				actividad = Permiso_persona.objects.filter(persona=_id)
...
def view_agregar_aprendiz(request):
	...
	if request.method == 'POST':
		...
		if formulario.is_valid() and formulario2.is_valid() and formulario3.is_valid() and formulario4.is_valid():
			...
			ape =formulario.cleaned_data['apellidos']
			tel =formulario.cleaned_data['telefono']
			img =formulario.cleaned_data['imgPerfil']
			...
			try:
				...
				if not query:
					try:
						u = User.objects.create_user(username=username,password=documento, email=email)
						persona = Persona.objects.create(documentoIdentidad=documento,nombres=nom,apellidos=ape,telefono=tel,imgPerfil=img,usuario=u)
...
def view_lista_programas(request):
	...
	if programa:
		...
		for x in programa:
			...
			actividad = Ficha.objects.filter(programa=_id)
...
def view_lista_roles(request):
	...
	if rol:
		...
		for x in rol:
			...
			actividad = Rol_persona.objects.filter(rol=_id)
	...
	if request.method == "POST":
		...
		if option_selected_nombreAprendiz and not option_selected_numFicha:
			...
			if queryDocumentoFicha:
				if not queryDocumentoFicha.exists():
...
def view_peticiones(request):
	...
	peticion = Permiso_persona.objects.raw('SELECT * FROM( SELECT pp.id,documentoIdentidad,nombres,apellidos,fecha,horaSalida,estado,numeroFicha,jornada,abreviacion FROM appsena_permiso_persona pp inner join appsena_persona p on(p.id = pp.persona_id) inner join appsena_permiso pe on (pp.permiso_id=pe.id) inner join appsena_persona_ficha fi on (p.id=fi.persona_id) inner join appsena_ficha f on (fi.ficha_id=f.id) inner join appsena_programa pro on (f.programa_id=pro.id) ) WHERE estado=("Aprobado") ') 
...
def view_ver_peticion(request, id_pet):
	...
	try:	
		...
		ficha = Persona_ficha.objects.filter(persona=permiso.persona.id)
		instru = Persona.objects.get(id=permiso.instructor)
	except:
		msjError = 'Este permiso no cumple con los parametros'
...
def view_aprobar_peticion(request, id_pet):
	...
	permiso.vigilante = request.user.persona.id
